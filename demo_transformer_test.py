# -*- coding: utf-8 -*-
"""Demo_Transformer_Test.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1QghdLcnRmRq1DdlvrVMMMg-uMbtlxJ0k

# Part 2: Network Testing

This tutorial demonstrates how to test the transformer-based model for the hysteresis loop prediction. The network model will be loaded through the saved state dictionary (.sd) file and the prediction results will be saved as (.csv) files.
...
def generate_square_subsequent_mask(sz1: int, sz2: int) -> Tensor:
    ...
    return torch.triu(torch.ones(sz1, sz2) * float('-inf'), diagonal=1).bool()
...
def main():
    ...
    state_dict = torch.load(dir + 'Transformer/Output/Model_Transformer.sd', map_location=device)
    ...
    with torch.no_grad():
        for in_B, in_F, in_T, in_D, out_H, out_H_head in test_loader:
            ...
            # Save results
            with open(dir+"Transformer/Output/pred.csv", "a") as f:
                np.savetxt(f, (outputs[:,:-1,:]*normH[1]+normH[0]).squeeze(2).cpu().numpy())
                f.close()
            with open(dir+"Transformer/Output/meas.csv", "a") as f:
